...
\begin{Scalability}
Unfortunately when working with large datasets, the amount of resources available on your computer are insufficient enough to make your script finish in a reasonable time. For these large datasets, it maybe useful to work on a high performance computing system, which will enable your data or command to be run simultaneously on >8 threads. It is possible to gain access to large computing resources through eResearch SA \url{https://www.ersa.edu.au}, enabling analysis of large datasets efficiently.\\ 

Their large machine called "Tizard" does not run using BASH, instead using a different command-line shell called csh (or the C shell). This shell is very similar to BASH, but can have different syntax elements, much like different programming languages.\\

e.g. \\
\begin{lstlisting}

# To set your path in bash
export PATH=/home/stevep/bin:$PATH

# To set your path in csh
setenv PATH $PATH:/home/stevep/bin

\end{lstlisting}

By having many users on one machine at one time means that there needs to be a system that determines who runs what and when. Tizard uses a "queuing" system, whereby users submit jobs to a queue, and then executed when a spot on the machine opens up. The command is executed using a PBS script (shown below). This script is written in csh (not bash) and contains extra parameters such as: \\
\begin{enumerate}
\item Wall Time: Time it takes to complete
...
This script will allow users to submit a BEAST job (Bayesian analysis of molecular sequences using MCMC) to tizard, and will email when its done. \\
